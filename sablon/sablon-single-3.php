<?php
/**
 * Haber detay — Şablon 3 (Sade)
 *
 * Son dakika şeridi bu şablonda kapalıdır (mevzu_should_show_son_dakika).
 */
...
<article class="icerik sablon-3<?php echo ' haber-' . (int) $post->ID; ?>" property="articleBody" data-title="<?php the_title_attribute(); ?>">

    <header class="sablon-3-header px-3 px-md-0 mb-3">
        ...
        <div class="d-flex flex-wrap align-items-center gap-2 gap-md-3 small text-body border-bottom pb-3 mb-0">
            ...
            <span class="ms-md-auto d-inline-flex flex-wrap align-items-center gap-2">
                ...
                <?php if ( in_array( 'yorum', $detaylar ) && comments_open() ) : ?>
                    <a href="#comments" class="ripple btn btn-outline-secondary btn-sm fw-bolder rounded-pill py-1 px-3 h-32 d-inline-flex align-items-center gap-2">
                        <i class="ri-chat-3-line me-1"></i><?php echo (int) get_comments_number(); ?>
                    </a>
                <?php endif; ?>
                <?php if ( in_array( 'paylas', $detaylar ) ) : ?>
                    <button type="button" class="ripple btn btn-outline-secondary btn-sm fw-bolder rounded-pill py-1 px-3 h-32 d-inline-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#paylas-sablon3">
                        <i class="ri-share-line fz-16 me-1"></i><?php esc_html_e( 'Paylaş', 'mevzu2' ); ?>
                    </button>
                <?php endif; ?>
    ...
    <?php if ( $the_native_video_html ) : ?>
        <div class="sablon-3-media mb-3"><?php echo $the_native_video_html; ?></div>
    <?php elseif ( has_post_thumbnail() ) : ?>
        <div class="sablon-3-media mb-3">
    ...
    <div class="sablon-3-body px-3 px-md-0">
...
<div class="modal fade" id="paylas-sablon3" tabindex="-1" aria-labelledby="paylasSablon3Label" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content p-lg-3">
            <div class="modal-header border-0">
                <h2 class="modal-title fw-bolder fs-5" id="paylasSablon3Label"><?php esc_html_e( 'Bu Yazıyı Paylaş', 'mevzu2' ); ?></h2>
