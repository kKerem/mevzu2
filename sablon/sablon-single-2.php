        ...
        <div class="col-auto small">
            <?php foreach ( $detaylar as $detay ) : ?>
                ...
                <span class="small" data-bs-toggle="tooltip" data-bs-title="<?php echo esc_attr( get_post_view() ); ?> kez okundu">
                    <i class="ri-eye-line fs-6 opacity-50"></i>
...
<div class="icerik mb-3 mb-lg-4 sablon-2<?php echo ' haber-'. $post->ID; ?>" property="articleBody"data-title="<?php the_title(); ?>">
    <?php
    ...
    if ( $the_native_video_html ) {
        echo '<div class="mb-0">' . $the_native_video_html . '</div>';
    } elseif ( has_post_thumbnail() ) {
        the_post_thumbnail('gorsel-thumbnail-col-8', ['title' => get_the_title(), 'loading'=>'lazy', 'class' => 'w-100 rounded-bottom-0']);
    } else {
        $ust_manset_id = (int) get_post_meta( get_the_ID(), 'ust_manset_gorseli_id', true );
        if ( $ust_manset_id ) {
            echo wp_get_attachment_image( $ust_manset_id, 'gorsel-thumbnail-col-8', false, ['title' => get_the_title(), 'loading' => 'lazy', 'class' => 'w-100 rounded-bottom-0'] );
        }
    }
    ?>
        
    <div class="bg-white rounded-bottom shadow-sm single-body p-2 p-md-3<?php echo (!has_post_thumbnail() && !$the_native_video_id) ? ' rounded-top' : '' ?>">
        <?php 
            ...
            <div class="row align-items-center justify-content-between w-100 g-0">
                <div class="col-auto">
                    ...
                    <div class="d-flex small single-bilgi align-items-center satir-1">
                        <?php if (in_array('yazar', $detaylar)) { echo $avatar_url; } ?>
                        <div>
        ...
        <?php if ( !in_category( 'kose-yazilari' ) && get_option('options_bizi_takip_edin_bolumu') == 1 ) echo do_shortcode('[takipedin]'); ?>
