...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) :
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( in_array( 'current-nav-item', $classes, true ) || in_array( 'current-menu-parent', $classes, true ) ) {
				$classes[] = 'active';
			}
			...
			if ( $this->has_children && 0 === $depth ) {
				...
				if ( $depth > 0 ) {
					...
					$atts['class'] = 'nav-link text-dark px-2';
				}
			}
			...
			// }
			// $icon_html = 'asdsad';
			$cat_icon = get_post_meta($item->ID, 'cat_icon', true);
			if($cat_icon) {
				// $icon_html = '<i class="'.$cat_icon.' fa-fw fa-lg me-2"></i>';
				$icon_html = '';
			}
			else {
				$icon_html = '';
